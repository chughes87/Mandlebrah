...
def Complex.mul (a b: Complex) : Complex :=
  ...
  let realComplex := a.real * b.i
  let complexReal := a.i * b.real
  let complex := a.i * b.i
...
def stepIn (bound : Float) (fam : Complex) : Nat → Nat → Complex → Nat
  | Nat.zero, currStep, _ => 0
  | Nat.succ maxSteps, currStep, z => match (OfComplex.ofComplex z ≥ bound : Bool) with
    | true => currStep
    | false => stepIn bound fam maxSteps (Nat.succ currStep) (step fam z)
...
def buildDimToComplex (xMax yMax : Nat) (cMin cMax : Complex) : Nat → Nat → Complex :=
  ...
  λ (px :Nat) => λ (py: Nat) =>
    Complex.mk (Float.ofNat px * xScalar) (Float.ofNat py * yScalar)
...
def newScene (px py : Nat) (cMin cMax : Complex) (maxSteps : Nat) : Array (Array Nat) := do
  ...
  let stepper := stepIn infinity 0 maxSteps 0
  Array.mapIdx scene λ idy dimx =>
    Array.mapIdx dimx λ idx elem=>
      stepper (dimToComplex idx idy)
